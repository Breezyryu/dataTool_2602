...
def get_col_idx(df, col_name, default_idx):
    """
    Helper to find column index by name, falling back to default index if not found.
...
def toyo_cycle_data(raw_file_path, mincapacity, inirate, chkir):
    ...
    if hasattr(tempdata, "dataraw") and not tempdata.dataraw.empty:
        ...
        # Keep original cycle
        if "OriCycle" not in Cycleraw.columns:
        ...
        if not Cycleraw.empty and Cycleraw.iloc[0]["Condition"] == 2:
             if len(Cycleraw) > 1 and Cycleraw.iloc[1]["TotlCycle"] == 1:
                  ...
                  mask_cond2 = Cycleraw["Condition"] == 2
                  Cycleraw.loc[mask_cond2, "TotlCycle"] -= 1
                  Cycleraw = Cycleraw.iloc[1:].reset_index(drop=True)
        ...
        chgdata = Cycleraw[cond_chg].set_index("TotlCycle")
        Chg = chgdata["Cap[mAh]"]
        Ocv = chgdata["Ocv"]
        ...
        dcir = Cycleraw[cond_dcir].copy()
        
        # 5. Discharge Data
        cond_dchg = (Cycleraw["Condition"] == 2) & (Cycleraw["Cap[mAh]"] > (mincapacity/60))
        Dchgdata = Cycleraw[cond_dchg].copy()
        Dchg = Dchgdata["Cap[mAh]"]
        Temp = Dchgdata["PeakTemp[Deg]"]
        DchgEng = Dchgdata["Pow[mWh]"]
        AvgV = Dchgdata["AveVolt[V]"]
        OriCycle = Dchgdata["OriCycle"]
                     ...
                     for std_col, alts in col_map.items():
                         for alt in alts:
                             if alt in dcirpro.columns:
                                 dcirpro.rename(columns={alt: std_col}, inplace=True)
                                 break
                     
                     if "Condition" in dcirpro.columns and "Voltage[V]" in dcirpro.columns:
        ...
        DchgEng = DchgEng.reset_index(drop=True)
        Chg2 = Chg2.reset_index(drop=True)
        ...
        df.NewData = pd.concat([Dchg, Ocv, Eff, Chg, DchgEng, Eff2, dcir_vals, Temp, AvgV, OriCycle], axis=1)
            ...
            for files in subfile:
                if "SaveEndData.csv" in files:
                    if os.stat(raw_file_path + "\\Restore\\" + files).st_size > 0 and mincapacity is not None:
                        # Optimization: Use usecols if possible, but structure suggests we need diverse columns
                        Cycleraw = pd.read_csv(raw_file_path + "\\Restore\\" + files, sep=",", skiprows=0, engine="c",
                                               header=None, encoding="cp949", on_bad_lines='skip')
                        ...
                        if ('PNE21' in raw_file_path) or ('PNE22' in raw_file_path):
                            # Vectorized division
                            Cycleraw[["DchgCap", "chgCap", "Curr"]] = Cycleraw[["DchgCap", "chgCap", "Curr"]] / 1000

                        if chkir:
                            dcirtemp = Cycleraw[(Cycleraw["Condition"] == 2) & (Cycleraw["volmax"] > 4100000)]
                            # Duplicate index handling is smarter here
                            dcir = dcirtemp.set_index("TotlCycle")["imp"] / 1000
                        ...
                        # 1s pulse, RSS DCIR
                            ...
                            # Alignment and Vectorized Calculation
                            if min_dcir_count > 0:
                                d1 = dcirtemp1.iloc[:min_dcir_count].reset_index(drop=True)
                                d2 = dcirtemp2.iloc[:min_dcir_count].reset_index(drop=True)
                                d3 = dcirtemp3.iloc[:min_dcir_count].reset_index(drop=True)
                                
                                cur1 = d1["Curr"]
                                cur2 = d2["Curr"]
                                vol1 = d1["Ocv"]
                                vol2 = d2["Ocv"] # Note: Original code uses same index 4(Ocv) for both
                                vol3 = d3["Ocv"] # Note: Original code uses index 4(Ocv)

                                # Vectorized DCIR calculation
                                # Avoid division by zero
                                valid_mask = (cur1 != 0) & ((cur1 - cur2) != 0)
                                
                                d1_imp = pd.Series(index=d1.index, data=np.nan)
                                d2_imp = pd.Series(index=d2.index, data=np.nan)
                                
                                d1_imp[valid_mask] = abs((vol3[valid_mask] - vol1[valid_mask]) / cur1[valid_mask] * 1000)
                                d2_imp[valid_mask] = abs((vol2[valid_mask] - vol1[valid_mask]) / (cur1[valid_mask] - cur2[valid_mask]) * 1000)
                                
                                # Assign back to original DF structure if needed, or keep for later
                                dcirtemp1["imp"] = d1_imp.values
                                dcirtemp2["imp"] = d2_imp.values

                        # SOC5,50 10s pulse DCIR
                        else:
                            # pulse 기준, 1분 이하 pulse 기준으로 산정
                            dcirtemp = Cycleraw[(Cycleraw["Condition"] == 2) & (Cycleraw["steptime"] <= 6000)].copy()
                            dcirtemp["dcir"] = dcirtemp["imp"] / 1000

                        # 필요한 모든 값을 한 번에 계산 - Pivot Table is efficient
                        ...
                        # Handling MultiIndex columns from pivot
                        # pivot_data columns are like ('DchgCap', 2), ('chgCap', 1)...
                        
                        # Extracting series safely
                        def get_pivot_series(metric, cond):
                            if (metric, cond) in pivot_data.columns:
                                return pivot_data[(metric, cond)]
                            return pd.Series(0, index=pivot_data.index)

                        Dchg = get_pivot_series("DchgCap", 2) / mincapacity / 1000
                        DchgEng = get_pivot_series("DchgEngD", 2) / 1000
                        Chg = get_pivot_series("chgCap", 1) / mincapacity / 1000
                        Ocv = get_pivot_series("Ocv", 3) / 1000000
                        Temp = get_pivot_series("Temp", 2) / 1000
                        
                        # ChgCap2 계산 (Vectorized shift)
                        ChgCap2 = Chg.shift(periods=-1)
                        
                        # Eff, Eff2, AvgV 계산 (Vectorized)
                        # FillNA or replace infs might be needed if Chg is 0
                        Eff = Dchg.div(Chg).replace([np.inf, -np.inf], np.nan)
                        Eff2 = ChgCap2.div(Dchg).replace([np.inf, -np.inf], np.nan)
                        AvgV = DchgEng.div(Dchg).div(mincapacity) * 1000
                        # OriCycle 생성
