...
def generate_simulation_full(ca_ccv_raw, an_ccv_raw, real_raw, ca_mass, ca_slip, an_mass, an_slip,
                             full_cell_max_cap, rated_cap, full_period, smoothing_mode='basic', denoise_strength=3.5, slope_window=2):
    """
    smoothing_mode:
        'basic': 기존 diff 방식
        'advanced_A': 입력 전압 스무딩 (Wavelet) -> dMSMCD (Advanced Pre-processing)
        'advanced_B': dV/dQ 계산 후 스무딩 (Wavelet + Savgol) (Advanced Post-processing)
    """
    # 라이브러리 경로 추가 및 import
    import sys
    sys.path.append('./smoothing_ref')
    try:
        from Lib_LKS_denoise import denoise, dMSMCD as slope_func
        from scipy.signal import savgol_filter
    except ImportError:
        smoothing_mode = 'basic' # 라이브러리 없으면 강제 기본 모드
    ...
    # 옵션 A: 입력 전압 전처리 스무딩
    if smoothing_mode == 'advanced_A':
        simul_full_an_volt = denoise(simul_full_an_volt, denoise_strength)
        simul_full_ca_volt = denoise(simul_full_ca_volt, denoise_strength)
        simul_full_real_volt = denoise(simul_full_real_volt, denoise_strength)
    ...
    # dV/dQ 계산 및 스무딩 적용
    if smoothing_mode == 'advanced_A':
        # Advanced A: dMSMCD 사용
        max_window = max(int(slope_window * 12 / 0.2), 1) # Crate 0.2 가정
        
        # dMSMCD는 series가 아닌 array 반환
        simul_full["an_dvdq"] = slope_func(simul_full.an_volt.values, max_window)['median'] / np.diff(simul_full_cap[:-1], prepend=simul_full_cap[0])
        simul_full["ca_dvdq"] = slope_func(simul_full.ca_volt.values, max_window)['median'] / np.diff(simul_full_cap[:-1], prepend=simul_full_cap[0])
        simul_full["real_dvdq"] = slope_func(simul_full.real_volt.values, max_window)['median'] / np.diff(simul_full_cap[:-1], prepend=simul_full_cap[0])
        
        # dQ 단위 보정 (백분율 용량 기준 dQ/dt)
        dq_dcap = np.diff(simul_full.full_cap, prepend=simul_full.full_cap.iloc[0])
        # 0 division 방지
        dq_dcap[dq_dcap==0] = 1e-6
        
        # dV/dQ = dV/dt / dQ/dt 인데 여기서는 간편하게 dMSMCD 결과를 dQ로 나눔 (근사)
        # dMSMCD는 dV/dn (index 기준 기울기). cap 간격이 0.1로 일정하므로 0.1로 나누면 dV/dCap.
        # 백분율 용량이므로 dV / d(SOC)
        
        simul_full["an_dvdq"] = simul_full["an_dvdq"] * 0.1 / dq_dcap
        simul_full["ca_dvdq"] = simul_full["ca_dvdq"] * 0.1 / dq_dcap
        simul_full["real_dvdq"] = simul_full["real_dvdq"] * 0.1 / dq_dcap

    else:
        # Basic & Advanced B: 기존 diff 방식
    ...
    # 옵션 B: 후처리 스무딩
    if smoothing_mode == 'advanced_B':
        # NaN 처리
        simul_full = simul_full.fillna(0)
        
        for col in ["an_dvdq", "ca_dvdq", "real_dvdq", "full_dvdq"]:
            # 1. Wavelet Denoise
            smoothed = denoise(simul_full[col].values, denoise_strength)
            # 2. Savgol Filter
            try:
                smoothed = savgol_filter(smoothed, window_length=int(len(smoothed)/20)|1, polyorder=3)
            except:
                pass
            simul_full[col] = smoothed
...
class WindowClass(QtWidgets.QMainWindow, Ui_sitool):
    ...
    def dvdq_graph(self, simul_full, min_params, rms):
        ...
        toolbar = NavigationToolbar(canvas, None)
        # Voltage Profile 그리기 (왼쪽 y축: 양극, 예측, 실측)
        line_ca, = ax1.plot(simul_full.full_cap, simul_full.ca_volt, "-", color = "r")
        line_pred, = ax1.plot(simul_full.full_cap, simul_full.full_volt, "--", color = "g")
        line_real, = ax1.plot(simul_full.full_cap, simul_full.real_volt, "-", color = "k")
        ax1.set_ylim(2.5, 4.7)
        ax1.set_xticks(np.linspace(-5, 105, 23))
        ax1.set_xlabel("SOC")
        ax1.set_ylabel("Cathode/Cell Voltage (V)")
        ax1.grid(which="major", axis="both", alpha=0.5)
        
        # 우측 y축: 음극 전압
        ax1_right = ax1.twinx()
        line_an, = ax1_right.plot(simul_full.full_cap, simul_full.an_volt, "-", color = "b")
        ax1_right.set_ylabel("Anode Voltage (V)", color="b")
        ax1_right.tick_params(axis='y', labelcolor='b')
        ax1_right.set_ylim(-0.1, 1.5)
        
        # 통합 legend (이전과 동일한 순서)
        ax1.legend([line_an, line_ca, line_pred, line_real], ["음극", "양극", "예측", "실측"])
        # dVdQ 그래프 그리기
